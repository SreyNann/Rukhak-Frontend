...
function AuthSignupMethod({ id, type, process, onClick }) {
  return (
    <Button
      type="submit"
      id={id}
    ...
    >
      {type === "Google" ? (
        ...
        <FacebookIcon sx={authTheme.facebookIcon} />
      ) : (
        <EmailIcon sx={authTheme.emailIcon} />
      )}
      {type === "Google" ? (
        <Typography variant="body2" marginLeft="0.5rem">
          {process} with google
        </Typography>
      ) : type === "Facebook" ? (
        <Typography variant="body2">{process} with Facebook</Typography>
      ) : (
        <Typography variant="body2">{process} with Email</Typography>
      )}
    </Button>
  );
}

export default AuthSignupMethod;
